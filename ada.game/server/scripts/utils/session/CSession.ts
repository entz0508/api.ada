...
export class CSession implements IPoolAbleObject
{
	...
	public mapping(data: Object): void
	{
		...
		this.m_dbShard      = CJson.safeStringParse( data, "dbShard", "", false);
	...
	public static async getSession(session: string, isTTLUpdate: boolean = true): Promise<CSession>
	{
		...
		const CSession: CSession = CSessionPool.alloc();
		...
		if (isTTLUpdate) {
			...
			await this.m_connect.setexAsync(this.getSessionIndexKey(CSession.uuid), CConfig.Env.SessionExpireSeconds, session);
		}
	 ...
	 * query.insert
	 ********************************************************************************************/
	public static async addSession(uuid: number, dbShard: number = -1): Promise<CSession>
	{
		const datenow: number = CTime.Util.getServerTimeStamp();
		...
		const instance: CSession = CSessionPool.alloc();
...
export class CSessionPool
{
	protected static ms_pool: CMemoryPool<CSession> = new CMemoryPool<CSession>(CSession);

	public static alloc(): CSession
	{
		const instance: CSession = this.ms_pool.alloc();
	...
	public static free(instance: CSession): void
	{
		if (instance) {
