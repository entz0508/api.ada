...
exports.add_routes = function (app) {
    ...
    app.post("/adb/mypage/mainHistory", ROUTE_MIDDLEWARE.LOGGED_IN_ACCOUNT, function (req, res) {
        ...
        try {
            ...
            MYSQL_ADB_ACCOUNT_CONN.procMypageHistory({
                ...
                //PRINT_LOG.error(__filename, API_PATH, "results[0].length : " + results[0].length);
    ...
    // 마이페이지 - 북마크 목록 - SORT
    app.post("/adb/mypage/bookmarklistMore", ROUTE_MIDDLEWARE.LOGGED_IN_ACCOUNT, function (req, res) {
        var API_PATH = req.route.path;
        var CLIENT_IP = COMMON_UTIL.getClientIP(req);
            ...
            if (sorttype == 0) {
                if (lastID != 0) {
                    if (lastID.indexOf('_') >= 0 && lastID.split('_').length === 2) {
                        lastID2 = lastID.split('_')[1];
                        lastID = lastID.split('_')[0];
                    }
                }
            }
            //else
            //{
            //    if (lastID.indexOf('_') >= 0) {
            //        lastID = 0;
            //    }
            //}

            //PRINT_LOG.info(__filename, API_PATH, " accountID parameter : " + accountID);
            //PRINT_LOG.info(__filename, API_PATH, "index : " + "000_0".indexOf('_'));
            // procMypageBookmark
            MYSQL_ADB_ACCOUNT_CONN.procMypageBookmarkmore({
                accountID: accountID,
                lastID: lastID,
                lastID2: lastID2,
                readcount: readcount,
                ...
                if (err) {
                    PRINT_LOG.error(__filename, API_PATH, " procMypageBookmarkmore, faile db, error" + err);
                    return PACKET.sendFail(req, res, ERROR_CODE_UTIL.RES_FAILED_DB, err, {});
                }

                PRINT_LOG.info(__filename, API_PATH, "results[0].length : " + JSON.stringify(results[0]));
                PRINT_LOG.info(__filename, API_PATH, "results[0].length : " + results[1].length);
                ...
                if ((0 < results[1].length)) {
                    ...
                    for (var i = 0; i < len; i++) {
                        verseInfo.bookmark_id = (sorttype == 1) ? results[1][i].tid : results[1][i].time_id + '_' + results[1][i].tid;
    ...
    // 마이페이지 - 하이라이트 목록 - SORT - OLD
    app.post("/adb/mypage/highlightlistMore", ROUTE_MIDDLEWARE.LOGGED_IN_ACCOUNT, function (req, res) {
        var API_PATH = req.route.path;
        var CLIENT_IP = COMMON_UTIL.getClientIP(req);
        ...
        try {
            ...
            MYSQL_ADB_ACCOUNT_CONN.procMypageHighlightmore({
                accountID: accountID,
                lastID: lastID,
                lastID2: lastID2,
                readcount: readcount,
                ...
                if (err) {
                    PRINT_LOG.error(__filename, API_PATH, " procMypageHighlightmore, faile db, error" + err);
                    return PACKET.sendFail(req, res, ERROR_CODE_UTIL.RES_FAILED_DB, err, {});
                }
                ...
                if ((0 < results[1].length)) {
                    var len = results[1].length;
                    for (var i = 0; i < len; i++) {
                        verseInfo.highlight_id = (sorttype == 1) ? results[1][i].tid : results[1][i].time_id + '_' + results[1][i].tid;
                        verseInfo.bible_id = results[1][i].bible_id;
                        verseInfo.chapter_id = results[1][i].chapter_id;
                        verseInfo.verse_id = results[1][i].verse_id;
                        verseInfo.verse_time = results[1][i].time;
