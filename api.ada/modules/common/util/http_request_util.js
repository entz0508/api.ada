...
var makeURL = function (baseURL, targetPath, targetPort) {
    if (443 === Number(targetPort))
        return "https://" + baseURL + "/" + targetPath;
    if (80 === Number(targetPort))
        return "http://" + baseURL + "/" + targetPath;
...
module.exports = {
    httpIsLoggedInAccount: function (clientUID, accessToken, callBack) {
        (function (clientUID, accessToken, callBack) {
            var url = makeURL(global.CONFIG.ADA_PLATFORM.URL, global.CONFIG.ADA_PLATFORM.PATH_IS_AUTH_ACCOUNT, Number(global.CONFIG.ADA_PLATFORM.PORT));
            request({
                uri: url,
                method: "POST",
                form: {
                ...
                if (error) {
                    PRINT_LOG.setErrorLog(__filename + "isLoggedInADAAccount, error request, url:" + url, error);
            ...
            if (err) {
                resData.msg = " error, http request, isLoggedInADAAccount, [" + httpResponseJson.url + "], " + __filename;
                //resData.msg = " error, http request, isLoggedInADAAccount, [ httpResponseJson.url ], " + __filename;
            } else {
