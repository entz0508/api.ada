...
module.exports = {
    ...
    LOGGED_IN_ACCOUNT: function (req, res, next) {
        ...
        if (!COMMON_UTIL.isValidClientUID(clientUID) || !COMMON_UTIL.isValidOS(os) || !COMMON_UTIL.isValidAccessToken(accessToken)) {
            if (!COMMON_UTIL.isValidClientUID(clientUID))
                PRINT_LOG.error(__filename, API_PATH, fnTitle + "clientUID " + clientUID);
            if (!COMMON_UTIL.isValidOS(os))
                PRINT_LOG.error(__filename, API_PATH, fnTitle + "os " + os);
            if (!COMMON_UTIL.isValidAccessToken(accessToken))
                PRINT_LOG.error(__filename, API_PATH, fnTitle + "accessToken " + accessToken);
    ...
    NOLOGGED_IN_ACCOUNT: function (req, res, next) {
        var fnTitle = " route_middleware.NOLOGGED_IN_ACCOUNT, ";
        var API_PATH = req.route.path;
        var clientUID, os, accountID, accessToken;
        if ("POST" === req.method) {
            clientUID = COMMON_UTIL.trim(req.body.client_uid);
