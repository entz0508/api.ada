...
export class CResponsePacket implements IPoolAbleObject
{
...
{
	protected static ms_pool: CMemoryPool<CResponsePacket> = new CMemoryPool<CResponsePacket>(CResponsePacket);

	public static alloc(session: string, uuid: number, dataVersion: number, packetSequenceNo: number, registTime: number, updateTime: number, expireTime: number): CResponsePacket
	{
		const instance: CResponsePacket = this.ms_pool.alloc();
	...
	public static free(instance: CResponsePacket): void
	{
		if (instance) {
