...
export class CNetworkConst
{
	public static Version = class
	{
		...
		{
			switch (state) {
				case CNetworkConst.Version.Error:                    { CDebug.logInfoFormat("[진입불가] 오류(클라이언트 버전이 존재하지 않거나. 내부 오류가 발생된 경우)"); return; }
				case CNetworkConst.Version.UnavailableVersion:       { CDebug.logInfoFormat("[진입불가] 허용되지 않는 버전"); return; }
				case CNetworkConst.Version.AvailableVersion:         { CDebug.logInfoFormat("[진입가능] 최신버전"); return; }
				case CNetworkConst.Version.RecommendUpdateVersion:   { CDebug.logInfoFormat("[진입가능] 업데이트 추천"); return; }
			}
		}
	};
	...
	public static Auth = class
	{
		...
		{
			switch (state) {
				case CNetworkConst.Auth.CreateByDeviceID:      { CDebug.logInfoFormat("디바이스 아이디로 신규 생성을 진행합니다."); return; }
				case CNetworkConst.Auth.LoadByDeviceID:        { CDebug.logInfoFormat("기존의 디바이스 아이디 계정을 로드 합니다."); return; }
				case CNetworkConst.Auth.CreateByMasterID:      { CDebug.logInfoFormat("마스터 아이디로 신규 생성을 진행합니다."); return; }
				case CNetworkConst.Auth.LoadByMasterID:        { CDebug.logInfoFormat("기존의 마스터 아이디 계정을 로드합니다."); return; }
				case CNetworkConst.Auth.LinkMasterID:          { CDebug.logInfoFormat("마스터 아이디를 링크합니다."); return; }
				case CNetworkConst.Auth.OverwriteFromMasterID: { CDebug.logInfoFormat("마스터 아이디의 계정으로 이전합니다."); return; }
			}
		}
	};
