			...
			<JarvisWidget editbutton={false} color="blue">
				<header>
					...
					<div className="widget-toolbar">
						...
						}])} onClick={this._toggleAdd}><i className={classnames({
							'fa fa-plus': !this.state.newTodo,
							'fa fa-times': this.state.newTodo
						})}/> Add
						</button>
				...
				<div>
					<div className="widget-body no-padding smart-form">
						...
						<TodoList type="Critical" title="Critical Tasks" icon="warning"
						          items={this.state.items.filter((item) => (item.type == 'Critical'))}/>

						<TodoList type="Important" title="Important Tasks" icon="exclamation"
						          items={this.state.items.filter((item) => (item.type == 'Important'))}/>

						<TodoList type="Completed" title="Completed Tasks" icon="check"
						          items={this.state.items.filter((item) => (item.type == 'Completed'))}/>
